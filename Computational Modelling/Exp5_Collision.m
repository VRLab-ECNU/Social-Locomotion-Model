...
for i = 1:2 % lateral distance
    for j = 1:3 % front-back
        for k = 1:4 % 0:facing the participant; 90:right side
            ...
            Blocker_Ori = 90*k - 90;
            EnergyMap = GetPhysicalMap(width/precision+1,Target(2)/precision+1,Blocker/precision,Blocker_Ori,m,n,a,b,c,precision);
            ...
            path = [rayTrace(T,EndPoint,StartPoint,RTsettings),[StartPoint(2);StartPoint(1)]];
...
dlmwrite('./results/path_multiple(collision).csv',pathall);
...
for i = 1:3 % lateral distance
    for j = 1:3 % front-back
        for k = 1:4 % 0:facing the participant; 90:right side
            ...
            l = sum(sqrt(h.^2+1));
            newpath = [ones(470,1)*LD,ones(470,1)*FB,ones(470,1)*Blocker_Ori,[1:470]',path3'];
            pathlengthR = [pathlengthR;[LD,FB,Blocker_Ori,arclength(path2(:,1),path2(:,2))]];
...
for i = 1:3 % lateral distance
    for j = 1:3 % front-back
        for k = 1:4 % 0:facing the participant; 90:right side
            ...
            l = sum(sqrt(h.^2+1));
            newpath = [ones(470,1)*LD,ones(470,1)*FB,ones(470,1)*Blocker_Ori,[1:470]',path3'];
            pathlengthL = [pathlengthL;[LD,FB,Blocker_Ori,arclength(path2(:,1),path2(:,2))]];
            for ii = 1:length(newpath)
                socialImpact = socialImpact + EnergyMap(round(newpath(ii,5)),newpath(ii,4));
            end
...
dlmwrite('./results/corM_c.csv',modelPrediction);