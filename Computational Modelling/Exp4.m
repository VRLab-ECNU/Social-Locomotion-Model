...
dlmwrite('./results/path_new.csv',pathall);
...
MLD = sortrows(MLD,1);
dlmwrite('./results/MLD_NEW.csv',MLD(:,2))
...
realdata = dlmread('/Users/chen_zhou/R_WorkingDirectory/Walking/PLR.csv');
realP = realdata(:,2);
...
% 
% a = 130.5; b = -130.7;
% modelP = 1./(1+exp(a+b*pathlengthRatio));
% modelPrediction = [realP,modelP];
% dlmwrite('./results/corP.csv',modelPrediction);
% 
% a = 60.36; b = -60.58;
% modelP = 1./(1+exp(a+b*pathlengthRatio));
% modelPrediction = [realP,modelP];
% dlmwrite('./results/corT.csv',modelPrediction);
% Wood Stairs
% stair = dlmread('/Users/chen_zhou/R_WorkingDirectory/Walking/finaldata.csv');
% xx = stair(:,1);yy = stair(:,2);
% a = -3.664; b = 0.1506;
% model = 1./(1+exp(a+b*xx));
% modelPrediction = [yy,model];
% dlmwrite('./results/corS.csv',modelPrediction);
