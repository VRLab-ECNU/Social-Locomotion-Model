...
{
  dat <- read.table(paste0("./Exp6/",subs[i]), header=FALSE, sep=";", na.strings="NA", dec=".", strip.white=TRUE)
...
# test <- datall[which(datall$Sub==1 & datall$Trial==2),]
...
write.table(finaldata,file = "finaldata.csv",row.names = FALSE,col.names = FALSE,sep = ",")
# finaldata$LD <- as.factor(finaldata$LD)
...
```{r line model prediction}
datcor <- read.table("./corI.csv",
   header=FALSE, sep=",", na.strings="NA", dec=".", strip.white=TRUE)
names(datcor) <- c("data","model")
datcor$model <- 1/(1+exp(5.647-6.144*datcor$model))
...
ggplot(data = predictgraph,aes(x=LD,y=p,group=interaction(IPD,Type))) + geom_line(aes(colour=IPD,linetype=Type),size=1) + geom_point(aes(colour=IPD,shape=Type),size=4.5) + geom_errorbar(aes(ymin=p-se, ymax=p+se,colour=IPD), width=3,size=1) + ylab("Probability of bypassing from right") + xlab("Lateral Distance (cm)")+ coord_cartesian(ylim=c(0, 1)) + scale_y_continuous(breaks=seq(0, 1, 0.2),expand=c(0,0)) + scale_x_continuous(breaks=seq(-60, 20, 20)) + theme_classic() + theme(axis.title = element_text(size=15, colour="black"),
  axis.text  = element_text(size=15, colour="black"),
  axis.line = element_line(size=0.5),
  axis.ticks = element_line(size=0.5),
  legend.text = element_text(size=15, colour="black"),
